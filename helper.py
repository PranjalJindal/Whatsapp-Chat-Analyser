...
def fetch_stats(selected_user,df):
    ...
    return df.shape[0],total_words,total_media,len(links)

def fetch_most_busy_users(df):
    x=df['user'].value_counts().head()
    df_new=round((df['user'].value_counts()/df.shape[0] )*100,2).reset_index().rename(columns={'count':'percent','users':'name'})
    return x,df_new
...
def emoji_count(selected_user,df):
    ...
    all_emojis = []
    for message in df['message']:
        all_emojis.extend([c for c in message if emoji.is_emoji(c)])
...
def week_activity(selected_user, df):
    ...
    week_df.columns = ['day_name', 'message_count']
    return week_df
